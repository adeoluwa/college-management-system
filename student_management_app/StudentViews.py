...
def student_apply_leave_save(request):
    if request.method != "POST":
        ...
        matric_number = request.POST.get("matric-number")
        ...
        student_obj = Students.objects.get(admin=request.user.id)
        try:
            leave_report = LeaveReportStudent(student_id=student_obj, date=date, leave_message=leave_message,full_name=full_name,email=email,faculty=faculty,department=department,matric_number=matric_number,phone_number=phone_number,address=address,session=session, leave_status=0)
            leave_report.save()
            messages.success(request, "Request Sent Successfully.")
            return redirect('student_apply_leave')
        except:
            messages.error(request, "Request Failed, try again")
            return redirect('student_apply_leave')
...
def student_course(request):
    return render(request, "student_template/student_course.html")